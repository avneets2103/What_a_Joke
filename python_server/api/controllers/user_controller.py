...
def humor_preprocess_joke(joke):
    # Replace slangs using the dictionary
    for slang, replacement in slang_dict.items():
        joke = joke.replace(slang, replacement)
    ...
    def average_vector(tokens, model):
        ...
        # Calculate the average
        return np.mean(vectors, axis=0)
    input = average_vector(tokens, w2v_model_humor)
    input = input.reshape(-1, 1)  # Reshape to (100, 1)
    input = input.reshape(1, 1, input.shape[0])  # Reshape to (1, 1, 100)
...
def joke_analyse(request):
    ...
    offenseScore = math.ceil((offense_model.predict(offense_processed_joke))*100)
    
    # Preprocess the joke
    humor_processed_joke = humor_preprocess_joke(joke)

    # Predict humor score
    humorScore = max(0, min(100, math.ceil((humor_model.predict(humor_processed_joke)))))
